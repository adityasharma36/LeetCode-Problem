...
public:
    ...
    void getAllSum(vector<vector<int>>& grid, int &cx, int &cy, priority_queue<int>&pq){
        // Push rhombus sum of size 0 (single cell)
        pq.push(grid[cx][cy]);

        int delta = 1;
        vector<pair<int,int>>v(4); // 4 points : A(top), B(right), C(bottom), D(left) wrt to each cell
        while(getAllVertices(grid, v, {cx, cy}, delta)){ // cx, cy: center coordinates
            pair<int,int>&A = v[0];
            pair<int,int>&B = v[1];
            pair<int,int>&C = v[2];
            pair<int,int>&D = v[3];
